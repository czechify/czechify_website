...
foreach (preg_split('/[.?!;()]/', $message) as $sKey => $sentence) {
            ...
            foreach ($splitSentence as $wordKey => $word) {
                ...
                if ($word == array_values(array_filter(explode(' ', $phrase)))[0]) {
                    ...
                    foreach ($indexesBeingChanged as $index1 => $index2) {
                        if ($index2 == count($splitPhrase) - 1) {
                            $wordIDsUsed[] = $wordId;
                            $splitSentence[$index1] = $phraseTo;
                        } else $splitSentence[$index1] = '';
    ...
    if (count($queueTMP)) {
        ...
        if (!($x)) {
            if (str_starts_with($mysqli->error, 'Duplicate entry')) {
    ...
    if ($punctuation[$sKey]) {
        if ($punctuation[$sKey] == '(') $punctuation[$sKey] = ' ' . $punctuation[$sKey]; else $punctuation[$sKey] .= ' ';
    }
...
function e($correctTranslations, $incorrectTranslations)
{
    return ($correctTranslations / ($incorrectTranslations + $correctTranslations)) * 100;
}
